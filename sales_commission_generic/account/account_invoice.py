	...
	def get_sales_commission(self):
		...
		for invoice in self:
			if invoice.user_id and invoice.move_type == 'out_invoice':
				...
				for record in commission_brw:
				
					if record.comm_type == 'mix':
						invoice_commission_ids = invoice.get_mix_commission(record, invoice)
					elif record.comm_type == 'partner':
						invoice_commission_ids = invoice.get_partner_commission(record, invoice)
					elif record.comm_type == 'discount':
						invoice_commission_ids = invoice.get_discount_commission(record, invoice)
					else:
						invoice_commission_ids = invoice.get_standard_commission(record, invoice)
