			...
			if order.user_id :
				...
				for record in commission_brw:
					if record.comm_type == 'mix':
						...
						invoice_commission_ids = order.get_mix_commission(record, order)
					elif record.comm_type == 'partner':
						invoice_commission_ids = order.get_partner_commission(record, order)
					elif record.comm_type == 'discount':
						invoice_commission_ids = order.get_discount_commission(record, order)
					else:
						invoice_commission_ids = order.get_standard_commission(record, order)
		return invoice_commission_ids
